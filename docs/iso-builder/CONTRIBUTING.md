...
- Ensure `tbox.wasm` and `wasm_exec.js` are populated as described in [README.md](https://github.com/tuna-os/iso-builder/blob/main/README.md#develop).
- Serve locally:
  ```sh
...
- Run tests (see [`native/README.md`](https://github.com/tuna-os/iso-builder/blob/main/native/README.md) for platform build prerequisites):
  ```sh
  cd native
  go vet ./...
  go test ./...
  ```
- `native/` also carries a [`.golangci.yml`](https://github.com/tuna-os/iso-builder/blob/main/.golangci.yml) (schema `version: "2"`, so it needs
