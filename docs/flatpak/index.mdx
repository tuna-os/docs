---
sidebar_position: 1
title: Flatpak
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# TunaOS Flatpaks

Install GNOME office apps directly from our Flatpak remote.

## 1. Add the remote

This is a one-time step:

```bash
flatpak remote-add tuna-os https://docs.tuna-os.org/flatpak/tuna-os.flatpakrepo
```

## 2. Install apps

### Python versions (stable)

<Tabs>
  <TabItem value="tables" label="🧮 Tables">
    ```bash
    flatpak install tuna-os org.tunaos.tables
    ```
  </TabItem>
  <TabItem value="decks" label="🎬 Decks">
    ```bash
    flatpak install tuna-os org.tunaos.decks
    ```
  </TabItem>
  <TabItem value="letters" label="📄 Letters">
    ```bash
    flatpak install tuna-os org.tunaos.letters
    ```
  </TabItem>
</Tabs>

Or install all three at once:

```bash
flatpak install tuna-os org.tunaos.tables org.tunaos.decks org.tunaos.letters
```

### 🦀 Rust versions (preview)

Native GTK4 rewrites — no Python or WebKit required. Lighter and faster, feature parity in progress.

<Tabs>
  <TabItem value="tables-rust" label="🧮 Tables (Rust)">
    ```bash
    flatpak install tuna-os org.tunaos.tables-rust
    ```
  </TabItem>
  <TabItem value="decks-rust" label="🎬 Decks (Rust)">
    ```bash
    flatpak install tuna-os org.tunaos.decks-rust
    ```
  </TabItem>
  <TabItem value="letters-rust" label="📄 Letters (Rust)">
    ```bash
    flatpak install tuna-os org.tunaos.letters-rust
    ```
  </TabItem>
</Tabs>
