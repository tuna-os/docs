...
<!-- ste-disable-file: this page is a copy of tuna-os/mandelbrot docs/FEATURES.md, kept identical to it on purpose. Its prose is authored in that repository, so a rewrite here is lost at the next copy and the fix belongs upstream. docs/mandelbrot/ is hand-authored, so the tree-level generated check does not cover it. -->
...
_Synced from [`tuna-os/mandelbrot` docs/FEATURES.md](https://github.com/tuna-os/mandelbrot/blob/main/docs/FEATURES.md)._
...
implementations — see the conformance report in the repo
(`tests/e2e/CONFORMANCE.md`) for the full details, including the two known
gaps (media encryption with Element Call is expected to work but is not yet
proven end to end, and Element Call's experimental MSC4354 "matrix 2.0" mode
