...
To build from source, install the Go version declared by [`go.mod`](https://github.com/tuna-os/tacklebox/blob/main/go.mod),
then run:
...
by [`go.mod`](https://github.com/tuna-os/tacklebox/blob/main/go.mod). Creating media with the installed CLI also requires:
